...
def applypatches(inputdir, bbfile):
    ...
    filelist = [os.path.basename(os.path.normpath(x)) for x in glob.glob(
        results_root + patch_root + inputdir + '*' + image_typ)]
    done_list = [os.path.basename(os.path.normpath(x)) for x in glob.glob(
        results_root + patched_image_root + inputdir + '*png')]
    ...
    for imf in filelist:
        ...
        #imf = filelist[bb[0]-1]
        im = imread(data_root + image_root + inputdir +
                    imf[:-len(image_typ)] + orig_typ)
        ...
        fig.savefig(results_root + patched_image_root +
                    inputdir + imf[:-len(image_typ)] + 'png')
        plt.close(fig)
        print imf
