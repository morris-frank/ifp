...
def extractpatches(inputdir, bbfile):
    ...
    #Get file list
    filelist = [os.path.basename(x) for x in glob.glob(data_root + image_root + inputdir + '*' + image_typ)]
    filelist = np.sort(filelist)

    if len(filelist) < len(bbmat):
        raise RuntimeError('The BoundingBox File has more rows than images exist!')
    ...
    for bb in bbmat:
        ...
        imsave(data_root + patch_root + inputdir + imf[:-len(image_typ)] + 'png', patch)
